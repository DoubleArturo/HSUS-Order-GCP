...
import { OrderStatus } from '../types/database.js';
const validateQuantities = (orderedItems, shippedTotals, incoming) => {
    ...
    for (const item of incoming) {
        ...
        if (newTotal > orderedQty) {
            throw new AppError(`Shipped qty for ${item.sku} exceeds ordered quantity`, 400, 'QTY_EXCEEDS_LIMIT');
        }
    }
};
