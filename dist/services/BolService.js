...
export class BolService {
     ...
     */
    static async getInitialBolData() {
        try {
     ...
     */
    static async getExistingBolData(poSkuKey) {
        try {
            ...
            // 2. Get Shipments
            const shipmentRes = await pool.query('SELECT * FROM shipments WHERE order_id = $1', [order.id]);
            ...
            const bols = shipments.map((s) => {
                ...
                if (Array.isArray(s.items)) {
                    qty = s.items.reduce((acc, item) => acc + (Number(item.qty) || 0), 0);
                }
                return {
        ...
        try {
            ...
            if (Array.isArray(bols)) {
                for (const bol of bols) {
                    ...
                    if (bol.bolNumber) {
                        await client.query(`INSERT INTO shipments (order_id, tracking_number, shipped_at, items)
                             VALUES ($1, $2, $3, $4)`, [orderId, bol.bolNumber, shippedAt, items]);
                    }
                }
            }
